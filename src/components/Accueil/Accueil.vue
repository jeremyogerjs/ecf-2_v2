  ...
  <div>
    ...
    <section class="product">
      <article :key="index" v-for="(product, index) in allProduct">
        <router-link class="product--overlay" :to="`/Product/${index + 1}`"
          ><span> View details</span></router-link
        >
        <span :id="index" @click="addToCart" class="product--overlay">
          add to cart
        </span>
        <img
          :src="require('../../assets/Media/' + `${product.image}`)"
          :alt="product.title"
        />
...
export default {
  ...
  methods: {
    addToCart(e) {
      ...
      if (
        this.$store.state.myCart.find(
          (element) => element == this.allProduct[e.target.id]
        )
      ) {
